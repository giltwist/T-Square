	...
	public static void setPoint(EntityPlayer player, boolean isRightClick) {
		if (!player.isSwingInProgress) {
			...
			if (point == null) {
				...
				player.addChatMessage(new TextComponentString("Ruler point " + whichPoint + " saved."));
	...
	public static void activate(EntityPlayer player) {
		if (!player.isSwingInProgress) {
			...
			if (!player.getEntityData().hasKey("TSquareRuler1X") || !player.getEntityData().hasKey("TSquareRuler2X")) {
				...
				player.addChatMessage(new TextComponentString("Distance:" +linearDistance));
				player.addChatMessage(new TextComponentString("* X-axis: " +xLength));
				player.addChatMessage(new TextComponentString("* Y-axis: " +yLength));
				player.addChatMessage(new TextComponentString("* Z-axis: " +zLength));
