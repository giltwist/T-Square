	...
	public static void activate(EntityPlayer player, boolean fullBlockState) {
		...
		if (targetBlock == null) {
			...
			if (isBlock && !player.isSneaking()) {
				...
				if (!TSquare.BLOCKBLACKLIST.contains(player.worldObj.getBlockState(targetBlock).getBlock())&&!TSquare.BLOCKBLACKLIST.contains(placematState.getBlock())) {
				player.worldObj.setBlockState(targetBlock, placematState);
				}else {
					player.addChatMessage(new TextComponentString("Error: Either target block or offhand material is on global blacklist"));
				}
			} else {
				//use normal means
