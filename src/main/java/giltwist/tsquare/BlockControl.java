...
public class BlockControl {
				...
				if (hasReplaceStateIfNeeded) {
					if (replaceMode.equalsIgnoreCase("all") || replaceMode.equalsIgnoreCase("air") || replaceMode.equalsIgnoreCase("milk") || replaceMode.equalsIgnoreCase("water") || replaceMode.equalsIgnoreCase("lava")
						...
						for (BlockPos b : toReplace) {
							...
							if (!TSquare.BLOCKBLACKLIST.contains(player.worldObj.getBlockState(b).getBlock()) && !TSquare.BLOCKBLACKLIST.contains(placemat)) {
						...
						// player.worldObj.markBlockRangeForRenderUpdate(minPosX,
	...
	public static void blendBlocks(EntityPlayer player, BlockPos[] toReplace, boolean includeAir) {
		...
		for (int i = 0; i < toReplace.length; i++) {
			...
			if (!TSquare.BLOCKBLACKLIST.contains(player.worldObj.getBlockState(toReplace[i]).getBlock())&&!TSquare.BLOCKBLACKLIST.contains(bestNeighbor[i].getBlock())){
